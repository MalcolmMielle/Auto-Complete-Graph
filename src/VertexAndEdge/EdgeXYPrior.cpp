...
Eigen::Vector2d g2o::EdgeXYPriorACG::getOrientation2D(
    const g2o::VertexXYPrior& from) const {
    // 	std::cout << "From " << from.estimate().toVector();
    g2o::VertexXYPrior* ptr =
        dynamic_cast<g2o::VertexXYPrior*>(this->vertices()[0]);
    g2o::VertexXYPrior* ptr2 =
        dynamic_cast<g2o::VertexXYPrior*>(this->vertices()[1]);
    assert(ptr != NULL);
    assert(ptr2 != NULL);
    ...
    if (ptr == &from) {
        ...
        assert(true == false &&
               "Weird, the original vertex wasn't found before");
    }
...
//		const VertexPointXYACG *v1 = static_cast<const VertexPointXYACG
//*>(_vertices[0]); 		const VertexPointXYACG *v2 = static_cast<const
// VertexPointXYACG *>(_vertices[1]); 		_error = _error +
// (v2->estimate() - v1->estimate()) - _measurement;
////	}
//
////	if(use_angle_error) {
...
////			g2o::VertexXYPrior *ptr =
///dynamic_cast<g2o::VertexXYPrior
///*>(vertex); /			angle_error += ptr->angleDiff(this); /
