...
int main() {
    ...
    double l = (pose_prior - pose_landmark)
                   .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    // I need to remove that 2 in the formula since we haven't added the
    // covariance together ! Hence no need to divided by two :)
    ...
    double factor = radius_around_landmark * radius_around_landmark /
                    (2 * std::sqrt(determinant));
    double mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    double pdf_collision = factor * std::exp(-mahalanobis_distance / 2);

    std::cout << "distance: " << 0 << " l: " << l << " score : " << score
              << " pdf " << pdf_collision << " and the factor is of " << factor
              << " and the det " << determinant << std::endl;

    cov_inv_2d << 1, 0, 0, 1;
    pose_prior << 0, 0;
    pose_landmark << 0, 0;
    scaling_factor_gaussian = 1;
    l = (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    // I need to remove that 2 in the formula since we haven't added the
    // covariance together ! Hence no need to divided by two :)
    ...
    std::cout << "Full collsision -> distance: " << 2 << " l: " << l
              << " score : " << score << " pdf " << pdf_collision
              << " and the factor is of " << factor << " and the det "
    ...
    mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "No collsision -> distance: " << 2 << " l: " << l
              << " score : " << score << " pdf " << pdf_collision
              << " and the factor is of " << factor << " and the det "
              << determinant << std::endl;
    ...
    mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "distance: " << 2 << " l: " << l << " score : " << score
              << " pdf " << pdf_collision << std::endl;
    ...
    mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "all mutlitplied by 2: distance: " << 1 << " l: " << l
              << " score : " << score << " pdf " << pdf_collision << std::endl;
    ...
    mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "all mutlitplied by 4: distance: " << 1 << " l: " << l
              << " score : " << score << " pdf " << pdf_collision << std::endl;
    ...
    mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "distance: " << 4 << " l: " << l << " score : " << score
              << std::endl;
    ...
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "distance: " << 4 << " l: " << l << " score : " << score
              << " l should be smaller here than before "
              << " pdf " << pdf_collision << std::endl;

    Eigen::Vector2d pose_mcl;
    pose_mcl << 0, 0;
    ...
    mahalanobis_distance =
        (pose_prior - pose_landmark)
            .dot(cov_inv_2d.inverse() * (pose_prior - pose_landmark));
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "Opposite side " << opposite_side << " distance "
    ...
    eigenvec << main_eigen(0), eigen(0), main_eigen(1), eigen(1);
    // For testing
    ...
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "distance: " << 4 << " l: " << l << " score : " << score
              << " l with rotation should be smaller "
              << " pdf " << pdf_collision << std::endl;
    ...
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "distance: " << 4 << " l: " << l << " score : " << score
              << " l with rotation should be even smaller "
              << " pdf " << pdf_collision << std::endl;
    ...
    pdf_collision = factor * std::exp(-mahalanobis_distance / 2);
    std::cout << "distance: " << 4 << " l: " << l << " score : " << score
              << " l with rotation should be even even smaller "
              << " pdf " << pdf_collision << std::endl;
}