...
class GraphMapFuserNode {
   protected:
    ...
    std::string points_topic, laser_topic, map_dir, odometry_topic,
        odometry_adjusted_topic;
    std::string file_format_map = ".JFF";
    std::string world_link_id, odometry_link_id, fuser_base_link_id,
        laser_link_id, init_pose_frame, gt_topic, bag_name, state_base_link_id;
   ...
   public:
    ...
    GraphMapFuserNode(ros::NodeHandle param_nh) : frame_nr_(0) {
        ...
        if (!matchLaser) {
                    ...
                    if (!use_tf_listener_) {
                        ...
                        points2OdomTfSub =
                            nh_.subscribe<sensor_msgs::PointCloud2>(
                                points_topic, 10,
                                &GraphMapFuserNode::GTLaserPointsOdomCallbackTF,
                                this);
                } else {
                    if (!use_tf_listener_) {
                        odom_sub_ =
            ...
            if (useOdometry) {
                ...
                ((void)0);  // Do nothing, seriously consider using a laser only
                            // callback (no odometry sync)
            }
        }
        if (initPoseFromGT) {
    ...
    void processFrame(pcl::PointCloud<pcl::PointXYZ>& cloud,
        ...
        tf_.sendTransform(tf::StampedTransform(
            tf_sensor_pose_, tplot, fuser_base_link_id, laser_link_id));
    ...
    {
        cout << "GT diff:"
             << (msg_in->header.stamp - odo_in->header.stamp).toSec() << endl;
        Eigen::Affine3d Tmotion;
        if (frame_nr_ == 0) {
