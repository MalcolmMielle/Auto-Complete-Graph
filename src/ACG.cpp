...
g2o::EdgeLinkXY_malcolm* AASS::acg::AutoCompleteGraph::addLinkBetweenMaps(
    ...
    for (auto link_edge : _edge_link) {
        ...
        } else if (v1 == link_edge->vertices()[1] &&
                   v2 == link_edge->vertices()[0]) {
            throw std::runtime_error("Edge link already added");
        }
    }
    ...
    // 			covariance_link(2, 2) = 13;//<- Rotation covariance link is more
    // than 4PI
    ...
    // linkObservation->vertices()[0] << " at " << __LINE__ << " " << __FILE__<<
...
// g2o::EdgeLinkXY_malcolm* AASS::acg::AutoCompleteGraphBase<Prior,
// g2o::VertexSE2Prior, EdgePrior>::addLinkBetweenMaps(const g2o::Vector2& pos,
// int from_id, int toward_id){ 	g2o::HyperGraph::Vertex* from_ptr =
//_optimizable_graph.vertex(from_id); 	g2o::HyperGraph::Vertex* toward_ptr =
//_optimizable_graph.vertex(toward_id);
//
//	g2o::VertexSE2Prior* ptr = dynamic_cast<g2o::VertexSE2Prior*>(from_ptr);
//	g2o::VertexLandmarkNDT* ptr2;
//	if(ptr != NULL){
//		g2o::VertexLandmarkNDT* ptr2 =
// dynamic_cast<g2o::VertexLandmarkNDT*>(toward_ptr); 		if(ptr2 ==
// NULL){ 			throw std::runtime_error("Pointers are not of
// compatible type. First pointer should be prior while the second is not a
// PointXY while it should");
//		}
//	}
//// 	else{
...
//// 			if(ptr2 == NULL){
//// 				throw std::runtime_error("Pointers are not of
/// compatible type. Second pointer is a SE2 while the first is not a PointXY");
//// 			}
//// 		}
//// 		else{
//// 			throw std::runtime_error("Pointers are not of compatible
/// type. No pointer point to a VertexSE2Prior"); / 		} / 	}
//	return addLinkBetweenMaps(pos, ptr, ptr2);
//}

std::vector<g2o::EdgeLinkXY_malcolm*>::iterator
AASS::acg::AutoCompleteGraph::removeLinkBetweenMaps(
...
void AASS::acg::AutoCompleteGraph::updateNDTGraph(
    ndt_feature::NDTFeatureGraph& ndt_graph,
    bool noise_flag,
    double deviation) {
    AutoCompleteGraphBase<AutoCompleteGraphPriorSE2, g2o::VertexSE2Prior,
                          g2o::EdgeSE2Prior_malcolm>::updateNDTGraph(ndt_graph,
                                                                     noise_flag,
                                                                     deviation);

    // TODO, update links using the newly found landmarks. No need to update the
    // rest obviously (<-HAHA that was so wrrrrongggg need to update it since
    // they moved :P!)
...
void AASS::acg::AutoCompleteGraph::testInfoNonNul(
    const std::string& before) const {
    AutoCompleteGraphBase<AutoCompleteGraphPriorSE2, g2o::VertexSE2Prior,
                          g2o::EdgeSE2Prior_malcolm>::testInfoNonNul(before);
...
bool AASS::acg::AutoCompleteGraph::linkAlreadyExist(
    g2o::VertexLandmarkNDT* v_pt,
    ...
    for (it; it != _edge_link.end(); ++it) {
        ...
        if (ptr == NULL) {
            ...
            throw std::runtime_error(
                "Links do not have the good vertex type. Prior lae");
        }
        g2o::VertexLandmarkNDT* ptr2 =
            dynamic_cast<g2o::VertexLandmarkNDT*>((*it)->vertices()[1]);
        if (ptr2 == NULL) {
            throw std::runtime_error(
...
bool AASS::acg::AutoCompleteGraph::noDoubleLinks() {
    ...
    for (it; it != _edge_link.end();) {
        ...
        if (ptr == NULL) {
            throw std::runtime_error(
                "Links do not have the good vertex type. Prior ndl");
        }
        g2o::VertexLandmarkNDT* ptr2 =
            dynamic_cast<g2o::VertexLandmarkNDT*>((*it)->vertices()[1]);
        if (ptr2 == NULL) {
        ...
        // 		for(auto ite2 = (*it)->vertices().begin(); ite2 !=
        // (*it)->vertices().end() ; ++ite2){
        // g2o::VertexSE2Prior* ptr_tmp =
        // dynamic_cast<g2o::VertexSE2Prior*>((*ite2));
        // g2o::VertexLandmarkNDT* ptr2_tmp =
        // dynamic_cast<g2o::VertexLandmarkNDT*>((*ite2));
        //
        // 			if(ptr_tmp != NULL){
        // 				std::cout << "Got a VertexSE2" <<
        // std::endl; 				ptr = ptr_tmp;
        // 			}
        // 			else if(ptr2_tmp != NULL){
        // 				std::cout << "Got a VertexPOINTXY" <<
        // std::endl; 				ptr2 = ptr2_tmp;
        // 			}
        // 			else{
        // 				throw std::runtime_error("Links do not have
        // the good vertex type");
        // 			}
        // 		}
...
void AASS::acg::AutoCompleteGraph::setKernelSizeDependingOnAge(
    g2o::OptimizableGraph::Edge* e,
    bool step) {
    AutoCompleteGraphBase<
        AutoCompleteGraphPriorSE2, g2o::VertexSE2Prior,
        g2o::EdgeSE2Prior_malcolm>::setKernelSizeDependingOnAge(e, step);

    g2o::EdgeLinkXY_malcolm* v_linkxy =
        dynamic_cast<g2o::EdgeLinkXY_malcolm*>(e);
    double age = -1;
    ...
    if (v_linkxy != NULL) {
        ...
        if (step == true) {
            if (age + _age_step <= _max_age && age + _age_step >= _min_age ||
                _max_age == -1) {
        ...
        // Keep going
        //  		std::cout << "kernel size : " << age << " age max " <<
        //  _max_age << " start " << _age_start_value << std::endl;
...
std::pair<int, int> AASS::acg::AutoCompleteGraph::optimize(int max_iter) {
    ...
    auto ret =
        AutoCompleteGraphBase<AutoCompleteGraphPriorSE2, g2o::VertexSE2Prior,
                              g2o::EdgeSE2Prior_malcolm>::optimize(max_iter);

    std::cout << "AFTER THE OPTIMIZATION CREATE" << std::endl;
    int count = countLinkToMake();
    int count2 = createNewLinks();
    // 			if(count != count2){
    // 				std::cout << "Weird different detection" <<
    // std::endl; 				throw std::runtime_error("ARF NOT GOOD
    // COUNT");
    // 			}
    // 			overCheckLinks();
...
void AASS::acg::AutoCompleteGraph::clearPrior() {
    ...
    AutoCompleteGraphBase<AutoCompleteGraphPriorSE2, g2o::VertexSE2Prior,
                          g2o::EdgeSE2Prior_malcolm>::clearPrior();

    // 			for(auto it = _edge_prior.begin() ; it != _edge_prior.end()
    // ;
    // ++it){ 				_optimizable_graph.removeVertex(it,
    // true);
    // 			}
    ...
    for (auto ite = idmapedges.begin(); ite != idmapedges.end(); ++ite) {
        // 				std::cout << "pointer " <<
        // dynamic_cast<g2o::EdgeSE2Prior_malcolm*>(*ite) << std::endl;
        assert(dynamic_cast<g2o::EdgeLinkXY_malcolm*>(*ite) == NULL);
    }
    std::cout << "IMPORTANT size " << _optimizable_graph.vertices().size()
              << std::endl;
...
void AASS::acg::AutoCompleteGraph::clearLinks() {
    ...
    // 			std::cout <<"Done final " << _nodes_prior.size() << " i " <<
    // i
    // <<std::endl;
