...
void gotGraphandOptimize(const ndt_feature::NDTGraphMsg::ConstPtr msg,
    ...
    std::cout << "WELLL HERE IT IS : " << occ_outt->info.origin.position
              << " ori " << occ_outt->info.origin.orientation << std::endl
              << std::endl;
    ...
    std::string file_outg =
        "/home/malcolm/ACG_folder/ACG_RVIZ_SMALL/occupancygrid_small_";
...
int main(int argc, char** argv) {
    ...
    ndt_graph_sub = nh.subscribe<ndt_feature::NDTGraphMsg>(
        "/ndt_graph", 10, boost::bind(&gotGraphandOptimize, _1, &oacg, visu));
