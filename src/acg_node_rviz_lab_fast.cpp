...
inline void printImages(AASS::acg::AutoCompleteGraph* oacg) {
    ...
    //
    //
    // 	std::cout << "WELLL HERE IT IS : " <<
    // occ_outt->info.origin.position_in_robot_frame << " ori " <<
    // occ_outt->info.origin.orientation << std::endl << std::endl;
    //
...
void gotGraphandOptimize(const ndt_feature::NDTGraphMsg::ConstPtr msg,
    ...
    // 	std::string file_out =
    // "/home/malcolm/ACG_folder/ACG_RVIZ_SMALL/oacg_before_";
    std::ostringstream convert;  // stream used for the conversion
    convert << graph.getNbNodes();
    // 	file_out = file_out + convert.str();
    // 	file_out = file_out + "nodes.g2o";
    ...
    // 	printImages(oacg);
    // 	std::string file_out_after =
    // "/home/malcolm/ACG_folder/ACG_RVIZ_SMALL/oacg_after_";
    // std::ostringstream convert_after;   // stream used for the conversion
    // convert_after << graph.getNbNodes(); 	file_out_after = file_out_after
    // + convert.str(); 	file_out_after = file_out_after + "nodes.g2o";
...
int main(int argc, char** argv) {
    ...
    // 		DEG2RAD(2.), 				//Rotation noise for
    // robot 		Eigen::Vector2d(0.05, 0.05), //Landmarks noise
    // Eigen::Vector2d(1, 0.01), //Prior noise 		DEG2RAD(2.), //Prior rot
    ...
    //  		Eigen::Vector2d(0.0005, 0.0001), //Robot translation
    //  noise 		DEG2RAD(2.), //Rotation noise for robot
    //  Eigen::Vector2d(0.5, 0.5), //Landmarks noise Eigen::Vector2d(1, 0.01),
    //  //Prior noise 		DEG2RAD(2.), //Prior rot
    //  Eigen::Vector2d(0.002, 0.002) //Link noise,
    //  	);
    //
    //  	acg.addPriorGraph(graph_prior);
    //  	std::string file_out =
    //  "/home/malcolm/ACG_folder/ACG_RVIZ_SMALL/acg_0_prior.g2o";
    //  	acg.getGraph().save(file_out.c_str());
    //  	std::cout << "saved to " << file_out << std::endl;
    ...
    //  		Eigen::Vector2d(0.0005, 0.0001), //Robot translation
    //  noise 		DEG2RAD(2.), //Rotation noise for robot
    //  Eigen::Vector2d(0.05, 0.05), //Landmarks noise Eigen::Vector2d(1, 0.01),
    //  //Prior noise 		DEG2RAD(2.), //Prior rot
    //  Eigen::Vector2d(0.2, 0.2)
    //  //Link noise,
    //  	);
    ...
    AASS::acg::AutoCompleteGraph oacg(g2o::SE2(0.2, 0.1, -0.1),
                                      "/home/malcolm/ACG_folder/param.txt");
    ...
    // 	ndt_graph_sub = nh.subscribe<ndt_feature::NDTGraphMsg>("ndt_graph", 10,
    // boost::bind(&gotGraph, _1, &acg, visu));
    ndt_graph_sub = nh.subscribe<ndt_feature::NDTGraphMsg>(
        "/ndt_graph", 100, boost::bind(&gotGraphandOptimize, _1, &oacg, visu));
    ...
    while (ros::ok() && flag == true) {
        ...
        ros::spinOnce();
        // 		visu.updateRvizV2();
        std::cout << oacg.getLinkEdges().size() << " and new node " << new_node
                  << std::endl;

        ros::Time future = ros::Time::now();
        std::cout << "future " << (future - timef).toSec() << " since "
                  << future << " " << timef << std::endl;
        // 		exit(0);

        if ((future - timef).toSec() >= 10 && oacg.getLinkEdges().size() > 5 &&
            new_node == true) {
            // 			std::cout << "Out " << (future - timef).toSec() << "
            // "<< (timef).toSec() << " " <<(future).toSec() <<std::endl;
            // exit(0);
            visu.updateRvizNoNDT();
            // 	oacg->initializeOptimization();
            // 	oacg->initialGuess();
            ...
            std::cout
                ...
                << std::endl
                << std::endl;
            std::cout << "Final number of nodes : "
                      << oacg.getGraph().vertices().size()
                      << " time : " << (future - time_begin).toSec()
                      << std::endl;
            std::cout << "Mean time for processing a node "
                      << node_process_time / cycles << std::endl;
            std::cout
                << "********************************************************"
                << std::endl
                << std::endl;
