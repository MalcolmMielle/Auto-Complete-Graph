...
void testMsg(const ndt_feature::NDTGraphMsg::ConstPtr msg) {
    ...
    // 			perception_oru::NDTMap* map_copy = new
    // perception_oru::NDTMap(new perception_oru::LazyGrid(resolution));

    perception_oru::NDTMap* map_copy;
    perception_oru::LazyGrid* lz;
    // 			bool good = perception_oru::fromMessage(lz, fuser.map,
    // m.map, frame);
...
void gotGraphandOptimize(const ndt_feature::NDTGraphMsg::ConstPtr msg,
                         ...
                         AASS::acg::VisuAutoCompleteGraph& visu) {
    // void gotGraphandOptimize(const ndt_feature::NDTGraphMsg::ConstPtr msg,
    // AASS::acg::AutoCompleteGraph* oacg){ 	try{
    new_node = true;
    // 	abort_f = true;
    std::cout << "Got a new graph " << std::endl;
    ...
    if (oacg->getRobotNodes().size() > 0) {
        ...
        // 		if(was_init == true){
        // 		int a;
        // 		std::cout << "PRESS ANYTHING NOT OPTIMISED YET" <<
        // std::endl; 		std::cin >>a;
        ...
        // 	std::ostringstream convert_after;   // stream used for the
        // conversion 	convert_after << graph.getNbNodes(); 	file_out_after =
        // file_out_after + convert.str(); 	file_out_after = file_out_after
        // + "nodes.g2o"; 	oacg->getGraph().save(file_out_after.c_str());
        ...
        // 	perception_oru::toOccupancyGrid(graph.getMap(), omap, 0.4,
        // "/world"); 	moveOccupancyMap(omap, graph.getT());
        //
...
int main(int argc, char** argv) {
    ...
    // 	ndt_graph_sub = nh.subscribe<ndt_feature::NDTGraphMsg>("/ndt_graph",
    // 1000, boost::bind(&testMsg, _1)); 	ndt_graph_sub =
    // nh.subscribe<ndt_feature::NDTGraphMsg>("/ndt_graph", 1000,
    // boost::bind(&gotGraphandOptimize, _1, &oacg));
