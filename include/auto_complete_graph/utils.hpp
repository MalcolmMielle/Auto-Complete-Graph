 ...
 */
inline double getAngle(const Eigen::Vector3d& base,
                       const Eigen::Vector3d& toward) {
 ...
 */
inline void translateFromRobotFrameToGlobalFrame(
    const Eigen::Vector3d& vec_input,
    const g2o::SE2& robot_frame_pose,
...
inline double distancePointLine(const Eigen::Vector2d& point,
                                const Eigen::Vector2d& p1_line,
                                const Eigen::Vector2d& p2_line) {
    double distance_point_to_line =
        std::abs(((p2_line[1] - p1_line[1]) * point[0]) -
                 ((p2_line[0] - p1_line[0]) * point[1]) +
                 (p2_line[0] * p1_line[1]) - (p2_line[1] * p1_line[0]));
    distance_point_to_line =
        distance_point_to_line /
        std::sqrt(((p2_line[1] - p1_line[1]) * (p2_line[1] - p1_line[1])) +
                  ((p2_line[0] - p1_line[0]) * (p2_line[0] - p1_line[0])));
    return distance_point_to_line;
}

inline std::tuple<double, Eigen::Vector2d> distancePointLine2(
...
inline std::tuple<double, Eigen::Vector2d> distancePointSegment(
    const Eigen::Vector2d& point,
    const Eigen::Vector2d& p1_line,
    const Eigen::Vector2d& p2_line) {
    Eigen::Vector2d AB = p2_line - p1_line;
    ...
    //			First, check to see if the nearest point on the line AB is beyond
    //B (as in the example above) by taking AB ⋅ BC. If this value is greater
    //than 0, it means that the angle between AB and BC is between -90 and 90,
