...
class VertexXYPrior : public g2o::VertexPointXYACG {
   public:
    ...
    std::vector<g2o::EdgeXYPriorACG*> getPriorEgdes() const {
        ...
        for (auto ite = edges.begin(); ite != edges.end(); ++ite) {
            ...
            if (ptr != NULL) {
                ...
                for (auto ite2 = edges_prior.begin(); ite2 != edges_prior.end();
                     ++ite2) {
                    assert(ptr != *ite2);
                }
    ...
    std::vector<std::pair<double, double>> getAnglesAndOrientations() const {
        ...
        if (edges_prior.size() > 1) {
            ...
            for (auto ite = edges_prior.begin(); ite != edges_prior.end() - 1;
                 ++ite) {
   ...
   private:
    std::pair<double, double> angle(const g2o::EdgeXYPriorACG& from,
                                    const g2o::EdgeXYPriorACG& to) const {
