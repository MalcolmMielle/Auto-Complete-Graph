...
class LocalizationPointer : public AASS::acg::Localization {
   public:
    ...
    // To check if a link between MCL and the landmark was already done using
    // the localization
    std::unordered_set<g2o::VertexPointXYACG*> _prior_linked_using_landmark;

    // Pose of the landmark in the equivalent robot_frame to this mcl frame when
    // it was detected -> used to translate to the corner.
    ...
    LocalizationPointer()
        : vertex_mcl_pose(NULL), edge_observation_from_mcl_pose(NULL) {}
    ...
    Eigen::Vector3d landmarkSeenByMCLInGlobalFrame() {
        ...
        Eigen::Vector3d mcl_frame_pose_vec =
            vertex_mcl_pose->localizationInGlobalFrame();
        g2o::SE2 mcl_frame_pose(mcl_frame_pose_vec);
        original_pose_in_mcl_frame_3d << original_pose_in_robot_frame(0),
            original_pose_in_robot_frame(1), 0;
        Eigen::Vector3d pose_inglobal_frame;
        translateFromRobotFrameToGlobalFrame(
            original_pose_in_mcl_frame_3d, mcl_frame_pose, pose_inglobal_frame);
