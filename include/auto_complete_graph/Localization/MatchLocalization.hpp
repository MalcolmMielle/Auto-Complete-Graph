...
class MatchLocalization {
   ...
   public:
    MatchLocalization(const cv::Point2f& p, cv::Point2f& r)
        : _prior(p), _landmark(r) {}
    ...
    void getNodes(const AutoCompleteGraphLocalization& acg) {
        Eigen::Vector2d pose2d_p;
        pose2d_p << _prior.x, _prior.y;
        auto prior_nodes = acg.getPrior()->getNodes();

        double norm_p = -1;

        for (auto prior_node_it_const = prior_nodes.begin();
             prior_node_it_const != prior_nodes.end(); prior_node_it_const++) {
            Eigen::Vector2d pose = (*prior_node_it_const)->estimate();
            //					Eigen::Vector2d pose2d; pose2d
            //<< pose(0), pose(1);
        ...
        std::vector<g2o::VertexLandmarkNDT*> landmark_nodes =
            acg.getLandmarkNodes();

        double norm_l = -1;
        pose2d_p;
        pose2d_p << _landmark.x, _landmark.y;

        for (std::vector<g2o::VertexLandmarkNDT*>::const_iterator
                 landmark_node_it_const = landmark_nodes.begin();
             landmark_node_it_const != landmark_nodes.end();
             landmark_node_it_const++) {
