...
namespace AASS
{
	...
	namespace acg
	{
		...
		protected:
			...
			/// is close enough to a NDT corner. The obervation is the corner observation
			/// from the robot to the NDT corner.

			// UNUSED
			bool _use_mcl_observation_on_prior = true;
			///@brief Add link in between ndt corner and prior corner based on a distance
			/// threshold.
   ...
   * between -10 and 10*/
		...
		public:
			AutoCompleteGraphLocalization(const g2o::SE2 &sensoffset,
				...
				: _gen(_rd()), AutoCompleteGraphBase<AutoCompleteGraphPriorXY,
													 g2o::VertexXYPrior,
																		  ...
																		  0)
			{
				...
				if (_use_links_prior_classic_ssrr && _use_mcl_observation_on_prior)
				{
					throw std::runtime_error(
						"ATTENTION: you used some funny parameters here young padawan. Link "
						"prior and MCL Observation edge on prior together, will lead to the "
			...
			//						const Eigen::Vector2d& pn,
			//						double rp,
			//						ndt_feature::NDTFeatureGraph*
			// ndt_graph
			//  					) : _gen(_rd()),
			//  AutoCompleteGraphBase<AutoCompleteGraphPriorXY, g2o::VertexXYPrior,
			//  g2o::EdgeXYPriorACG>(sensoffset, tn, rn, ln, pn, rp, ndt_graph){
			//
			//	        if(_use_links_prior_classic_ssrr &&
			//_use_mcl_observation_on_prior){ 		        throw
			//std::runtime_error("ATTENTION: you used some funny parameters here young
			// padawan. Link prior and MCL Observation edge on prior together, will lead to
			// the prior being linked in two different ways. Take care.");
			//	        }
			//        }
			...
			AutoCompleteGraphLocalization(const g2o::SE2 &sensoffset,
				...
				: _gen(_rd()), AutoCompleteGraphBase<AutoCompleteGraphPriorXY,
													 g2o::VertexXYPrior,
													 g2o::EdgeXYPriorACG>(sensoffset, tn, rn, ln, pn, rp)
			{
			...
			void print() const
			{
				...
				std::cout << "Number of localization nodes: " << _nodes_localization.size()
						  << std::endl;
				...
				///@brief Use the covariance of MCL into the link between corner in submaps
				/// and prior. NOT WORKING :)
				std::cout << "Use MCL covariance in links: " << _use_covariance_for_links
						  << std::endl;
				///@brief Add an observation between mcl poses and prior corner if the prior
				/// is close enough to a NDT corner. The obervation is the corner
				/// observation from the robot to the NDT corner.
				std::cout << "Use MCL corner observatino to directly link to the prior: "
				...
				///@brief do we want to scale the score a little bit to help out ?
				std::cout << "Scaling factor for gaussians: " << _scaling_factor_gaussian
						  << std::endl;
			...
			const std::vector<g2o::EdgePriorObservation *> &getPriorObservations() const
			{
				return _edge_prior_observation;
			}
			...
			const std::set<g2o::EdgeNDTCellObservation *> &getNDTCellObservations() const
			{
				return _edges_ndt_cell_observation;
			}
			...
			//	    std::tuple<g2o::VertexXYPrior, g2o::EdgeXYPriorACG>
			// addWeakAssociation(const g2o::SE2& transformation, g2o::HyperGraph::Vertex*
			// v1);
   ...
   */
			...
			/** Others **/
			/**@brief set the vertex in the prior that is going to be the reference point
   *for the localization. return NULL if failed, otherwise return the pointer
   *to the vertex that was chosen
   **/
   ...
   * Localization and the new nodes are added to the graph. If the g2o graph has
   * 4 nodes, only nodes 5 to last node of the NDT graph are added to it. Add
   * NDT-corners and Robot poses.
   */
			void updateNDTGraph(
			...
			/**
   * @brief Incrementally update the NDTGraph UPDATED TO THE NEW VERSION :)
   * All new nodes and new localization are added to the graph. Old information
   * is not touched.
   */
			void addNDTGraph(
				const auto_complete_graph::GraphMapLocalizationMsg &ndt_graph_localization);
			...
			void extractCornerNDTMap(const std::shared_ptr<perception_oru::NDTMap> &map,
